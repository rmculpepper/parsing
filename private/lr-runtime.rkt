        ...
        (cond [(and (eq? (pstate-accept st) 'true))
              ...
              [else
               (for ([red (pstate-reduce st)])
                 (reduce st tsk* red))
               (unless (hash-empty? (pstate-shift st))
